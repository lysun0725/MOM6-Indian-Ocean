...
import sys
sys.path.append('../MOM6-examples/tools/python/MIDAS')
...
topo_path='/lustre/lysun/GEBCO_2020.nc'
...
xstart=np.where(topo_grid.lonq>xbnds[0])[0][0]
#xend=np.where(topo_grid.lonq>xbnds[1]-360.)[0][0]
xend=np.where(topo_grid.lonq>xbnds[1])[0][0]
...
Depth=applyIce9(model_grid.D,ni/4,nj/4)
